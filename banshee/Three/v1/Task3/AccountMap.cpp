...
Class:				CSCI204
Lab:				Fri 0830 - 1030
Descrip:			AccountMap Class Function Definitions
	...
	if(!input.good()){
		...
		if(length > 0){		
			...
			while(!input.eof()){
				...
				accounts.insert(std::pair<const char* const, Account>(tempName, temp));
...
void AccountMap::displayData(){
	...
	std::multimap<const char*, Account, CompareCharArrays>::iterator i = accounts.begin();
